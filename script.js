...
function showNotes() {
  ...
  notesObj.forEach(function(element, index) {
    ...
    <div class="notebox">
      <div class="notedatecontainer"><h6>${d.getDate() + " " + months[d.getMonth()]
      }</h6></div>
     
      <textarea class="form-control noteheading" id="noteheading${index + 1
      }" onchange="saveNote(${index + 1})" maxlength="25">
${element.title}</textarea>
      <div class="notecontainer">
        <textarea class="notedata form-control" style="height: 190px" id="notedata${index + 1
      }" onchange="saveNote(${index + 1})">
${element.text}</textarea>
      </div>
      <div class="noteaction">
        <i class="fa fa-floppy-o savebutton" id="${index + 1
      }" onclick="saveNote(this.id)"></i>
        <i class="fa fa-trash deletebutton" id="${index + 1
